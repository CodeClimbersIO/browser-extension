...
const parseCanva = (): DesignProject | undefined => {
  const projectName = (
    document.head.querySelector('meta[property="og:title"]') as HTMLMetaElement
  ).content
...
const parseFigma = (): DesignProject | undefined => {
  ...
  const projectName = (
    document.querySelector('span[data-testid="filename"]') as HTMLElement
  ).innerText
  return {
    category: 'designing',
    editor: 'Figma',
...
const DEBOUNCE_DEFAULT_TIMEOUT = 1000 * 60 * 2 // 2 minutes

function debounce(func: () => void, timeout = DEBOUNCE_DEFAULT_TIMEOUT) {
